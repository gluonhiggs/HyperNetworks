...
class Logger:
    ...
    def _best_slice_point(self, mask, length):
        # Use fixed length search if any shape consistency is detected AND length is provided
        shape_is_known = (self.task.in_out_same_size or 
            self.task.all_out_same_size)
    ...
    def _postprocess_solution(self, prediction, x_mask, y_mask):  # prediction must be example, color, x, y
        ...
        for example_num in range(self.task.n_test):
            x_length = None
            y_length = None
            # Use predicted lengths if any shape consistency is detected
            shape_is_known = (self.task.in_out_same_size or
                             self.task.all_out_same_size)
            if shape_is_known:
