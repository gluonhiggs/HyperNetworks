                ...
                if all([higher_naxes >= naxes for higher_naxes, naxes in zip(higher_dims, dims)]):
                    ...
                    for dim, (higher_naxes, naxes) in reversed(list(enumerate(zip(higher_dims, dims)))):
                        if higher_naxes > naxes:
                            ...
                            shape_is_known = (x.multitensor_system.task.in_out_same_size or 
                                             x.multitensor_system.task.all_out_same_size)
                            if shape_is_known and dim==3:  # be careful aggregating the x axis
                            ...
                            elif shape_is_known and dim==4:  # be careful aggregating the y axis
                                # expand/contract masks to make the dims the same as higher_x
                                masks = x.multitensor_system.task.masks
                                masks = 1-(1-masks[...,0])*(1-masks[...,1])
                                for i in range(sum(higher_dims[1:3])):  # insert color and direction dims
...
def direction_share(dims, x, weights, pre_norm=True, use_bias=False):
    ...
    n_directions = dims[3] + dims[4]
    if n_directions == 0: return x
    direction_dim = sum(dims[:2])  # Correct for variable position

    # Stack to tensor with D at -2
    x_stacked = torch.stack(torch.unbind(x, dim=direction_dim), dim=-2)  # ... D C
    z_stacked = torch.stack(torch.unbind(z, dim=direction_dim), dim=-2)

    # Circulant coeff matrix (d1, d2)
    coeffs_list = [1, 0.2, 0.4, 0.2, 1, 0.2, 0.4, 0.2]
    C = torch.zeros(8, 8, device=x.device)
    for d1 in range(8):
        C[d1] = torch.tensor([coeffs_list[(d2 - d1) % 8] for d2 in range(8)])

    # Stack weights: w_mats (d1, d2, Cin, Cout), b_mats (d1, d2, Cout)
    Cin, Cout = x.shape[-1], x.shape[-1]
    w_mats = torch.stack([torch.stack([w[0] for w in row], dim=0) for row in weights], dim=0)
    b_mats = torch.stack([torch.stack([w[1] for w in row], dim=0) for row in weights], dim=0) if use_bias else torch.zeros(8, 8, Cout, device=x.device)

    # Vectorized affine: batched over d1, d2 pairs
    aff = torch.einsum('...ji, kjio -> ...kjo', z_stacked, w_mats)  # ... d1 d2 cout

    if use_bias:
        # Expand b_mats to match leading dims: (1, ..., d1, d2, cout) -> expand to aff.shape
        expand_shape = list(aff.shape[:-3]) + [-1, -1, -1]
        aff += b_mats.view([1] * len(aff.shape[:-3]) + list(b_mats.shape)).expand(expand_shape)

    # Weight and sum over d2 (dim=-2)
    weighted = aff * C[None, :, :, None]  # Broadcast C over leading and cout
    delta = torch.sum(weighted, dim=-2)  # ... d1 cout

    # Add back and unstack
    x_stacked += delta
    return torch.stack(torch.unbind(x_stacked, dim=-2), dim=direction_dim)
@multitensor_systems.multify
@add_residual
