...
class Task:
    ...
    def _predict_solution_shapes(self):
        ...
        # Check if all output x (height) are the same
        self.all_out_same_x = len({shape[1][0] for shape in self.shapes if shape[1]}) == 1
        # Check if all output y (width) are the same
        self.all_out_same_y = len({shape[1][1] for shape in self.shapes if shape[1]}) == 1
        ...
        if self.in_out_same_size:
            ...
            output_shapes = [shape[1] for shape in self.shapes[:self.n_train] if shape[1]]
            # get all shapes including input and output
            all_in_out_shapes = [shape for shape in self.shapes[:self.n_train]]
            # INSERT_YOUR_CODE
            # all_in_out_shapes is a list of [input_shape, output_shape] pairs
            # We want all_shapes to be a flat list of all input and output shapes
            all_shapes = []
            for pair in all_in_out_shapes:
                for shape in pair:
                    if shape is not None:
                        all_shapes.append(shape)
            if not output_shapes:
                # Fallback to max dimensions from all shapes if no output shapes
                max_x, max_y = self._get_max_dimensions()
                for shape in self.shapes[self.n_train:]:
                    shape[1] = [max_x, max_y]
                return
            print(f"output_shapes: {output_shapes}")
            max_output_x = max(shape[0] for shape in all_shapes)
            max_output_y = max(shape[1] for shape in all_shapes)
            print(f"max_output_x: {max_output_x}, max_output_y: {max_output_y}")
            
            # Find second largest from OUTPUT shapes only
            output_x_values = [shape[0] for shape in all_shapes]
            output_y_values = [shape[1] for shape in all_shapes]
            
            unique_x_values = sorted(set(output_x_values), reverse=True)
            unique_y_values = sorted(set(output_y_values), reverse=True)
            
            print(f"unique_x_values: {unique_x_values}, unique_y_values: {unique_y_values}")
            if len(unique_x_values) > 1:
                second_max_x = unique_x_values[1]  # Second largest
            else:
                second_max_x = 0
                
            if len(unique_y_values) > 1:
                second_max_y = unique_y_values[1]  # Second largest
            else:
                second_max_y = 0
            if self.all_out_same_x:
                print(f"all_out_same_size_x: {self.all_out_same_x}")
            if self.all_out_same_y:
                print(f"all_out_same_size_y: {self.all_out_same_y}")
            print(f"second_max_x: {second_max_x}, second_max_y: {second_max_y}")
            x_overhead = max_output_x - second_max_x
            y_overhead = max_output_y - second_max_y
            for shape in self.shapes[self.n_train:]:
                shape[1] = [min(max_output_x + x_overhead, 30), min(max_output_y + y_overhead, 30)]
...
def preprocess_tasks(split, task_nums_or_task_names):
    ...
    solutions = None if split == "test" else json.load(open(f'{data_dir}/arc-agi_{split}_solutions.json'))
